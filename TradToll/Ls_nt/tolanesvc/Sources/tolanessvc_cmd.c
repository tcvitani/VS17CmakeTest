...
PRIVATE void CmdExecuteSendRef( ACOM_CNX_HANDLE hCnx, char * pcPlaza, char * pcLane );
PRIVATE void CmdExecuteRunTreatment( ACOM_CNX_HANDLE hCnx );
PRIVATE void CmdExecuteStatus( ACOM_CNX_HANDLE hCnx );
PRIVATE void CmdExecuteSetRef( ACOM_CNX_HANDLE hCnx, char * pcPlaza, char * pcLane, char * pcPath );
...
PROTECTED void CmdReceive( ACOM_CNX_HANDLE hCnx, DWORD dwMsgSize, char * pcMsg )
{
    ...
    else if ( CmdCheck( "SENDREF", pcMsg, dwMsgSize, 2, tpcParams ) )
        CmdExecuteSendRef( hCnx, tszParams[0], tszParams[1] );
    ...
    else if ( CmdCheck( "STATUS", pcMsg, dwMsgSize, 3, tpcParams ) )
        CmdExecuteSetRef( hCnx, tszParams[0], tszParams[1], tszParams[2] );
...
PRIVATE void CmdExecuteCommandList( ACOM_CNX_HANDLE hCnx )
{
    ...
    CmdResponse( hCnx, "COMMANDLISTITEM SENDREF,PLAZA|LANE" );
    CmdResponse( hCnx, "COMMANDLISTITEM SETPARAM,NAME|VALUE" );
    CmdResponse( hCnx, "COMMANDLISTITEM SETREF,PLAZA|LANE|REFPATH" );
...
PRIVATE void CmdExecutePrompt( ACOM_CNX_HANDLE hCnx )
{
    ...
    CmdResponse( hCnx, "PROMPTITEM TITLE    : %s", TOLANESSVC_PROMPT_TITLE );
    CmdResponse( hCnx, "PROMPTITEM VERSION  : %s", TOLANESSVC_PROMPT_VERSION );
    CmdResponse( hCnx, "PROMPTITEM DATE     : %s", TOLANESSVC_PROMPT_DATE );
    CmdResponse( hCnx, "PROMPTITEM COMMENTS : %s", TOLANESSVC_PROMPT_COMMENTS );
...
PRIVATE void CmdExecuteSendRef( ACOM_CNX_HANDLE hCnx, char * pcPlaza, char * pcLane )
{
    DWORD dwIndex;
    DWORD dwErr;
    DWORD dwPlaza = atol(pcPlaza);
    DWORD dwLane = atol(pcLane);
    TOLANESSVC_LANE * psLane;
    ...
    {
        psLane = &gsSvcWork.psList[dwIndex];
        if ( psLane->bBusy && ( psLane->dwPlaza == dwPlaza ) && ( psLane->dwLane == dwLane ) )
        {
            if ( psLane->bConnected )
            {
                dwErr = LaneSendRefMsg( dwIndex );
                if ( dwErr == NO_ERROR )
                    CmdResponse( hCnx, "SENDREFOK" );
                else
                    CmdResponse( hCnx, "SENDREFERROR %u,Cannot send reference message", dwErr );
            }
            else
                CmdResponse( hCnx, "SENDREFERROR %u,The lane is not connected", ERROR_INVALID_PARAMETER );
            break;
        }
    }

    if ( dwIndex >= gsSvcWork.sParmWork.dwMaxLanes )
        CmdResponse( hCnx, "SENDREFERROR %u,The lane does not exist", ERROR_INVALID_PARAMETER );
...
PRIVATE void CmdExecuteRunTreatment( ACOM_CNX_HANDLE hCnx )
{
    ...
    if ( ! SetEvent( gsSvcWork.hWakeupEvent ) )
    {
        dwErr = GetLastError();
...
PRIVATE void CmdExecuteSetRef( ACOM_CNX_HANDLE hCnx, char * pcPlaza, char * pcLane, char * pcPath )
{
    ...
    if ( gsSvcWork.bDatabaseMissing )
    {
        ...
        for ( dwIndex = 0 ; dwIndex < gsSvcWork.sParmWork.dwMaxLanes ; dwIndex ++ )
        {
            ...
            if ( psLane->bBusy && ( psLane->dwPlaza == dwPlaza ) && ( psLane->dwLane == dwLane ) )
            {
                if ( strlen( pcPath ) < sizeof( psLane->szRefFile ) )
                {
                    strcpy_s( psLane->szRefFile, MAX_PATH, pcPath );
                    if ( psLane->bConnected )
                    {
                        dwErr = LaneSendRefMsg( dwIndex );
                        if ( dwErr == NO_ERROR )
                            CmdResponse( hCnx, "SETREFOK" );
                        else
                            CmdResponse( hCnx, "SETREFERROR %u,Cannot send reference message", dwErr );
                    }
                    else
                        CmdResponse( hCnx, "SETREFOK" );
                }
                else
                    CmdResponse( hCnx, "SETREFERROR %u,The file path is too long", ERROR_INVALID_PARAMETER );
        ...
        if ( dwIndex >= gsSvcWork.sParmWork.dwMaxLanes )
            CmdResponse( hCnx, "SETREFERROR %u,The lane does not exist", ERROR_INVALID_PARAMETER );

        LeaveCriticalSection( &gsSvcWork.sCritical );
    }
    else
        CmdResponse( hCnx, "SETREFERROR %u,This command cannot be executed when the database is online", ERROR_INVALID_DATA );
}
