...
#define MSG_BACKUP_FILE_NB_FIELDS MSG_HEADER_NB_FIELDS + 4
...
MSG_DECLARE_REF(MSG_BACKUP_FILE, MSG_BACKUP_FILE_NB_FIELDS)
...
PROTECTED void MSG_BACKUP_FILE_Init(void)
{
    ...
    union MSG_Field LOCAL_MSG_BACKUP_FILE[MSG_BACKUP_FILE_NB_FIELDS] =  
    {
		{ MSG_FIELD_HEADER(MSG_BACKUP_FILE, MSG_BACKUP_FILE_CD) },

        { MSG_FIELD_DWORD,   MSG_OFFSET(MSG_BACKUP_FILE, body.begin_file_number), 0UL,99999999UL},
		{ MSG_FIELD_DWORD,   MSG_OFFSET(MSG_BACKUP_FILE, body.end_file_number), 0UL,99999999UL},
    ...
    MSG_INIT_REF(MSG_BACKUP_FILE,                    LOCAL_MSG_BACKUP_FILE);
}
...
PUBLIC struct MSG_BACKUP_FILE *MSG_BACKUP_FILE_New(void)
{
    // la variable MSG_BACKUP_FILE de type union MSG_Field[] est globale au module 
    // et a ete declare et initialise par les macros MSG_DECLARE_REF et MSG_INIT_REF
    return MSG_New(MSG_BACKUP_FILE, sizeof(struct MSG_BACKUP_FILE), "MSG_BACKUP_FILE");
}



// suppression COMPLETE d'un message de BACKUP BY FILE
...
PUBLIC BOOL MSG_BACKUP_FILE_Delete_All(struct MSG_BACKUP_FILE *p_backup_file)
{
    return MSG_Delete_All(&p_backup_file);
}
...
// passe a la moulinette une structure de donnée "struct MSG_BACKUP_FILE"
// a travers des parcours récursif des tableaux de description des champs
// de type union MSG_Field[](cf variable MSG_BACKUP_FILE de MSG_BACKUP_FILE_New
// et MSG_BACKUP_FILE_Init)
// La variable p_msg contient en retour le mesasge sous forme ASCII
PUBLIC BOOL MSG_BACKUP_FILE_Write(struct MSG_BACKUP_FILE *p_backup_file, 
                               BYTE *p_msg, 
                               DWORD msg_size_max, 
                               DWORD *final_msg_size)
{
    *final_msg_size = MSG_Write(p_backup_file, p_msg, msg_size_max);
...
// de type struct MSG_BACKUP_FILE
PUBLIC BOOL MSG_BACKUP_FILE_Read(struct MSG_BACKUP_FILE *p_backup_file, 
                              BYTE *p_msg, 
                              DWORD msg_size_max, 
                              DWORD *final_msg_size)
{
    *final_msg_size = MSG_Read(p_backup_file, p_msg, msg_size_max);
...
PUBLIC BOOL MSG_BACKUP_FILE_New_Record(HLIST *hList)
{
    return MSG_New_Record(hList, 
                         (MSG_Compare) MSG_HEADER_Compare_CD, 
                         (PVOID) MSG_BACKUP_FILE_CD, 
 			              MSG_BACKUP_FILE, 
                          sizeof(struct MSG_BACKUP_FILE), "MSG_BACKUP_FILE");
}
