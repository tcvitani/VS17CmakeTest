	 ...
	 case AFF_TYP_RESET_HW :
		 ...
		 if (SrvEstDemandeur (AFF[ident_aff].service[M_AFF_AFFICHAGE], p_msg_recu->entete.neutre.bl_retour) == TRUE)
		 {
               ...
               if( p_etat->aff_hs == FALSE)
               {
				  ...
				  AffFichierDebug(ident_aff, "AFF_ANI: AFFReceptionAffichage() => Hardware reset not supported by this device!");
               }
                                  ...
                                  struct_aff_message *p_message)
{
   ...
   if( etat == TRUE)
   {
      AffFichierTrace( ident_aff, "AFF_ANI: Display backlight is ON");
   }
   else
   {
      AffFichierTrace( ident_aff, "AFF_ANI: Display backlight is OFF");
   }
