...
PROTECTED void    * WINAPI PublieBALToQueue(char *nom_bal, unsigned long nb_messages_en_attente_max, mbx2slot_wrapper * psMbx2slot)
{
	...
	__try
	{
		...
		NOYAU_INIT_TACHE(
			...
			TRUE,                                          // tache active
			THREAD_PRIORITY_NORMAL,                        // priorité initiale
			0,                                             // taille de pile préallouée
			(LPTHREAD_START_ROUTINE)(Mbx2SlotThread),      // fonction thread
			(PVOID)(psMbx2slot),                           // paramètre de tache
			NULL,                                          // valeur mise à jour par LanceTache()
			"");
