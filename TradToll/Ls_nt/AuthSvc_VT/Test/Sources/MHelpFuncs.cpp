...
QByteArray MHelpFuncs::StrToByteArray(const QString & sData)
{
	...
	for(i=0; i < iLen; i++)
	{
		...
		if(cByteVal.toAscii()>=32)
			baTemp = QByteArray(1,cByteVal.toAscii()); 
...
{
	return str.replace('§',STR_NEWLINE_FOR_DSK);
}
...
QString MHelpFuncs::getVersionInfo()
{
	...
	VerQueryValue(pVersion, (unsigned short *)QString("\\").utf16(), (LPVOID*)&pFixInfo, &length);
...
void MHelpFuncs::DateTime2SysTime(QDateTime *ptm, struct _SYSTEMTIME * lpSystemTime)
{
	lpSystemTime->wYear = ptm->date().year();
	lpSystemTime->wMonth = ptm->date().month();
	lpSystemTime->wDay = ptm->date().day();

	lpSystemTime->wHour = ptm->time().hour();
	lpSystemTime->wMinute = ptm->time().minute();
	lpSystemTime->wSecond = ptm->time().second();
	lpSystemTime->wMilliseconds = ptm->time().msec();
