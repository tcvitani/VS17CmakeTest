...
// RETURNS:		-
///////////////////////////////////////////////////////////////////////////////
PROTECTED void TlmUtil_FreeMailbox(noyau_bal_id mailbox_id)
{
								   ...
								   )
{
	...
	ExitAlloue((struct_neutre**)&(message), sizeof(struct_tlm_message), gsTLM[instance_id].pool);

	message->entete.neutre.bl_retour = gsTLM[instance_id].ani_bal;
	message->entete.service = service;
	message->entete.type_message = (enum_srv_type)message_type;

	MInstTracer_FichierTrace(instance_id, "TLM_UTIL: TlmUtil_SendMessage() => destination=%d, service=%d, message type=%d",
				 destination_mailbox_id, message->entete.service, message->entete.type_message);

	send_result = Envoie(destination_mailbox_id, gsTLM[instance_id].ani_bal, (struct_neutre*)message);
							   ...
							   noyau_bal_id bal_demandeur)
{
	...
	if (SrvEstDemandeur(gsTLM[inst_id].service[service_id], bal_demandeur) == FALSE)
		if (SrvAjouteDemandeur(gsTLM[inst_id].service[service_id], bal_demandeur, 0L) != NULL)
			ret = SRV_TYP_DEBUT_ACQ;

	EnvoiAcquittement(inst_id, bal_demandeur, service_id, ret);
							 ...
							 noyau_bal_id bal_demandeur)
{
	...
	pSrvData = SrvRechercheDemandeur(gsTLM[inst_id].service[service_id], bal_demandeur, NULL, NULL);
	if (pSrvData == NULL)
	{
		EnvoiAcquittement(inst_id, bal_demandeur, service_id, SRV_TYP_FIN_NACQ);
		return FALSE;
	}

	SrvEnleveDemandeur(gsTLM[inst_id].service[service_id], &pSrvData);
	EnvoiAcquittement(inst_id, bal_demandeur, service_id, SRV_TYP_FIN_ACQ);
									 ...
									 enum_espion_nature nature)
{
	...
	if (SrvEstDemandeur(gsTLM[inst_id].service[M_SRV_ESPION], bal_demandeur) == FALSE)
	{
		pSrvData = SrvAjouteDemandeur(gsTLM[inst_id].service[M_SRV_ESPION], bal_demandeur, sizeof(struct_srv_espion));
		if (pSrvData != NULL)
		{
			pSrvData->sens = sens;
	...
	EnvoiAcquittement(inst_id, bal_demandeur, M_SRV_ESPION, ret);
...
PROTECTED void EnvoiAcquittement(IN tlm_inst_id inst_id,
								 ...
								 IN enum_srv_type type_message)
{
	...
	ExitAlloue((struct_neutre **)&(p_msg_emis), sizeof(struct_tlm_message), gsTLM[inst_id].pool);

	p_msg_emis->entete.neutre.bl_retour = gsTLM[inst_id].ani_bal;
	...
	ExitEnvoie(bal_dest, gsTLM[inst_id].ani_bal, (struct_neutre *)(p_msg_emis));
}

PROTECTED void EnvoiEtat(tlm_inst_id inst_id, struct_srv_etat *psEtat)
{
	struct_tlm_message p_msg;
	struct_tlm_envoi_service data = { inst_id, &p_msg };

	p_msg.entete.neutre.bl_retour = gsTLM[inst_id].ani_bal;
	...
	SrvPourChaqueDemandeur(gsTLM[inst_id].service[M_SRV_ETAT], EnvoiDemandeur, &data);
}

PROTECTED void EnvoiCmdState(tlm_inst_id instance_id, struct_tlm_display *message, enum_srv_service	service, enum_tlm_type	message_type)
{
	struct_tlm_message p_msg;
	struct_tlm_envoi_service	callback_param = { instance_id, &p_msg };

	if (!SrvEstVide(gsTLM[instance_id].service[M_TLM_MESSENGER_SERVICE]))
	{
		MInstTracer_FichierTrace(instance_id, "EnvoiCmdState() - message");
		// Filling the structure
		p_msg.entete.neutre.bl_retour = gsTLM[instance_id].ani_bal;
		p_msg.entete.service = service;
		p_msg.entete.type_message = (enum_srv_type)message_type;

		memcpy(&p_msg.srv.srv_tlm, message, sizeof(struct_tlm_display));

		SrvPourChaqueDemandeur(gsTLM[instance_id].service[M_TLM_MESSENGER_SERVICE], EnvoiDemandeur, &callback_param);
	}

}


PROTECTED void EnvoiMessageEspion(IN tlm_inst_id inst_id,
								  ...
								  IN unsigned int iLength)
{
	...
	struct_tlm_envoi_service data = { inst_id, &p_msg_emis };
	...
	SrvPourChaqueDemandeur(gsTLM[inst_id].service[M_SRV_ESPION], EnvoiDemandeurEspion, &data);
}

/**/
/*****************************************************************************/
/*SYNTAX: void TlmUtil_SendMessageToIos(short siInstId,                      */
...
PROTECTED void TlmUtil_SendMessageToIos(short siInstId,
										...
										struct_tlm_message *pvParam)
{
	struct_tlm_message *output_message = NULL;

	// Allocating the memory
	ExitAlloue((struct_neutre **)(&output_message), sizeof(struct_tlm_message), gsTLM[siInstId].pool);

	// Filling the message type field
	output_message->entete.type_message = (enum_srv_type)eTypeMessage;

	switch (eTypeMessage)
	{
	case TLM_MESSAGE_SET_REQUESTED:
	{
		memcpy(&output_message->srv.srv_tlm.u.display_srv, &pvParam->srv.srv_tlm.u.display_srv, sizeof(struct_tlm_service));
	}
	break;
	...
	MInstTracer_FichierTrace(siInstId, "TLM_ANI: EnvoiIOS() => type_message '%d' ", output_message->entete.type_message);

	// Sending the message
	if (Envoie(gsTLM[siInstId].ios_bal, gsTLM[siInstId].ani_bal, (struct_neutre *)output_message) != NOYAU_OK)
	{
...
{
	struct_tlm_envoi_service	*p_data = (PVOID)Param;
	struct_tlm_message			*p_msg_emis;

	ExitAlloue((struct_neutre **)(&p_msg_emis), sizeof(struct_tlm_message), gsTLM[p_data->inst_id].pool);

	memcpy(p_msg_emis, p_data->p_msg, sizeof(struct_tlm_message));
	ExitEnvoie(BalDemandeur, gsTLM[p_data->inst_id].ani_bal, (struct_neutre *)(p_msg_emis));
...
PRIVATE BOOL WINAPI EnvoiDemandeurEspion(IN PVOID Param,
										 ...
										 IN PVOID DataDuDemandeur)
{
	struct_tlm_envoi_service *p_param = Param;
		...
		(p_data->sens == SRV_ESPION_ENTRANT_SORTANT || p_data->sens == p_param->p_msg->srv.espion.sens))
	{
		...
		ExitAlloue((struct_neutre **)(&p_msg_emis), sizeof(struct_tlm_message), gsTLM[p_param->inst_id].pool);

		// recopie du message
		*p_msg_emis = *(p_param->p_msg);

		ExitEnvoie(BalDemandeur, gsTLM[p_param->inst_id].ani_bal, (struct_neutre *)(p_msg_emis));
	}
										...
										void *pMsg)
{
	...
	MInstTracer_FichierTrace((short int)instance_id, "TLM_IOS: TlmUtil_SendMessageToAni() => message_type '%d'", message->entete.type_message);

	send_result = Envoie(gsTLM[instance_id].ani_bal,
						 gsTLM[instance_id].ios_bal,
