...
PRIVATE DWORD CONF_PV_Get_Display_Zone_Suite(CHAR *Tab0);
													 ...
													 OUT DWORD *zone_id)
{
	...
	*zone_id = 0;
	...
	if ((plazanum != 0) && (disp_id != 0))
	{
		/* ID_PLAZA, ID_LANE_GROUP */
	...
	{
		/* RTO_ZONE */
		strcpy_s(TabInfo[0].info_0, sizeof(TabInfo[0].info_0), "RTO_ZONE");
		...
		while (Sortie == FALSE)
		{
			...
			*pt_num_section = sauv_num_section;
			...
			if (num_section > 0)
			{
				...
				if (bRet == TRUE)
					ind_resul = CONF_PV_Get_Display_Zone_Suite(TabInfo[0].info_1);
				else
		...
		if (ind_resul > 0)
		{
			*zone_id = atoi(Resultat[Index].info_0);
			Index = Index + 1;
...
DWORD CONF_PV_Get_Display_Zone_Suite(CHAR *Tab0)
{
	DWORD i;
	BOOL Sortie = FALSE;
	DWORD x, y;
