													...
													OUT char curr_label[MSG_PV_MAX_CURRENCY_LABEL])
{
	...
	*curr_id = 0;
	...
	/* *curr_id, curr_label */
	strcpy_s(TabInfo[0].info_0, sizeof(TabInfo[0].info_0), "ID_CURRENCY");
	strcpy_s(TabInfo[1].info_0, sizeof(TabInfo[1].info_0), "CDE_LABEL_CURRENCY");
	...
	while (Sortie == FALSE)
	{
		...
		bRet = INI_Recupere(IN "T_CURRENCY.ini", num_section, nbcriteres, TabInfo);
	...
	if (ind_resul > 0)
	{
		*curr_id = atoi(Resultat[Index].info_0);
		strcpy_s(curr_label, MSG_PV_MAX_CURRENCY_LABEL, Resultat[Index].info_1);
		Index = Index + 1;
