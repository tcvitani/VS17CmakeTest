...
EXPORT enum_instance_result WINAPI AFFLance( char * pcKey, char * pcBalNam, noyau_bal_id * piBalId )
{
	...
	if (REG_Lire_Entier(CSR_REG_KEYi_ROOT, pcKey, AFF_REG_KEYv_DELAY_BETWEEN_CMD, &AFF[ident_aff].dwDelayCmdTimeout) != ERROR_SUCCESS)
	{
		AFF[ident_aff].dwDelayCmdTimeout = 0;
		AffFichierDebug(ident_aff, "AFF_INIT ***** AffLance() => Missing Regystry parameter [%s] inexistent, set to 0", AFF_REG_KEYv_DELAY_BETWEEN_CMD);
	}
	...
	// MaxLinkError
	if (REG_Lire_Entier(CSR_REG_KEYi_ROOT, pcKey, AFF_REG_KEYv_MAX_LINK_ERROR, &AFF[ident_aff].MaxLinkError) != ERROR_SUCCESS)
	{
		AffFichierDebug(ident_aff, "AFF_INIT ***** AffLance() => Erreur registre : [%s]:%s", pcKey, AFF_REG_KEYv_MAX_LINK_ERROR);
		return INST_INIT_ERR_REGISTRE;
	}
	...
	if (AlloueChrono(&AFF[ident_aff].uiChronoPoll,
					"TEST DISPLAY") != NOYAU_OK)
		return INST_INIT_ERR_LANCE;

	if (AlloueChrono(&AFF[ident_aff].uiChronoDelayCmd,
		"CHRONO_DELAY_CMD") != NOYAU_OK)
		return INST_INIT_ERR_LANCE;
...
EXPORT enum_instance_result WINAPI AFFArret(noyau_bal_id iBalId)
{
	...
	// Free timer
	LibereChrono(&AFF[ident_aff].uiChronoPoll);
	LibereChrono(&AFF[ident_aff].uiChronoDelayCmd);
