...
EXPORT enum_instance_result WINAPI AFFLance( char * pcKey, char * pcBalNam, noyau_bal_id * piBalId )
{
	...
	char                  pcFichierFormat[MAX_PATH + 1] = { 0 };  // REG : Chemin du fichier de format
	char 				  pcPoolName[MAX_PATH + 1]		= { 0 };       // REG : Id de pool
	char				  pcPortCfg[MAX_PATH + 1]		= { 0 };        // REG : Chaine de config du port serie
	char                  pcProtocol[MAX_PATH]			= { 0 };				// Protocol layer
	...
	// ID
	if (REG_Lire_Entier(CSR_REG_KEYi_ROOT, pcKey, AFF_REG_KEYv_ID_ADDRESS, &AFF[ident_aff].dwId_address) != ERROR_SUCCESS)
	{
		if (AFF[ident_aff].dwId_address < 0 || AFF[ident_aff].dwId_address > 9)
		{
			AffFichierDebug(ident_aff, "AFF_INIT ***** AffLance() => Erreur registre : [%s]:%s", pcKey, AFF_REG_KEYv_ID_ADDRESS);
			return INST_INIT_ERR_REGISTRE;
		}
	}

	if (REG_Lire_Entier(CSR_REG_KEYi_ROOT, pcKey, AFF_REG_KEYv_ID_ADDRESS_LOWER, &AFF[ident_aff].dwId_address_lower) != ERROR_SUCCESS)
	{
		if (AFF[ident_aff].dwId_address_lower < 0 || AFF[ident_aff].dwId_address_lower > 9)
		{
			AFF[ident_aff].dwId_address_lower = 0;
			AffFichierDebug(ident_aff, "AFF_INIT ***** AffLance() => Erreur registre : [%s]:%s", pcKey, AFF_REG_KEYv_ID_ADDRESS_LOWER);
			return INST_INIT_ERR_REGISTRE;
		}
	}

	if (REG_Lire_Entier(CSR_REG_KEYi_ROOT, pcKey, AFF_REG_KEYv_ID_ADDRESS_UPPER, &AFF[ident_aff].dwId_address_upper) != ERROR_SUCCESS)
	{
		if (AFF[ident_aff].dwId_address_upper < 0 || AFF[ident_aff].dwId_address_upper > 9)
		{
			AFF[ident_aff].dwId_address_upper = 0;
			AffFichierDebug(ident_aff, "AFF_INIT ***** AffLance() => Erreur registre : [%s]:%s", pcKey, AFF_REG_KEYv_ID_ADDRESS_UPPER);
			return INST_INIT_ERR_REGISTRE;
		}
	}
