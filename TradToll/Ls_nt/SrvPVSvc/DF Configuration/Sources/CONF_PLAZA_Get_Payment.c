												   ...
												   OUT CHAR pay_name[MSG_PV_MAX_PAYMENT_LABEL])
{
	...
	*pay_id = 0;
	...
	/* ID_PAYMENT_TYPE, CDE_LABEL_PAYMENT_TYPE */
	strcpy_s(TabInfo[0].info_0, sizeof(TabInfo[0].info_0), "ID_PAYMENT_TYPE");
	strcpy_s(TabInfo[1].info_0, sizeof(TabInfo[1].info_0), "CDE_LABEL_PAYMENT_TYPE");
	...
	while (Sortie == FALSE)
	{
		...
		bRet = INI_Recupere(IN "T_PAYMENT_TYPE.ini", num_section, nbcriteres, TabInfo);
	...
	if (ind_resul > 0)
	{
		*pay_id = atoi(Resultat[Index].info_0);
