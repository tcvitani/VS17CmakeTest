...
#ifndef DBIF_MAP
{
    ...
    do
    {
        ...
        wOraErr = OCIHandleAlloc( 
                psDb->hEnv,             // Environment for the error handle
        ...
        wOraErr = OCIHandleAlloc( 
                psDb->hEnv,             // Environment for the server handle
        ...
        wOraErr = OCIHandleAlloc( 
                psDb->hEnv,             // Environment for the service context handle
        ...
        wOraErr = OCIHandleAlloc(
                psDb->hEnv,             // Environment for the session handle
...
#ifndef DBIF_MAP
{
    ...
    do
    {
        ...
        wOraErr = OCIHandleAlloc(
                psDb->hEnv,             // Environment for the statement handle
        ...
        if ( wOraErr != OCI_SUCCESS )
        {
            DBSetErrDb( psDb, psDb->hEnv, OCI_HTYPE_ENV, wOraErr, TRUE );
            break;
        }
...
PRIVATE HMODULE WINAPI DbLoadLibrary()
{
   ...
   // static HMODULE hDbLib = NULL;
	...
	if ( ghDbLib == NULL )
    {
		...
		//SetErrorMode(0);
		ghDbLib = LoadLibrary( szDbLib );

		if(ghDbLib==NULL)
		{
			char msg[200] = {0};
			sprintf_s(msg, sizeof(msg), "DB_MAP: Error loading DBIFORA OCI DLL:[%s]!",szDbLib );
			MessageBox(NULL,msg, "DBIFORA ERROR!!!", MB_OK|MB_ICONEXCLAMATION);
			ExitProcess(0xBAD);
		}
    }
