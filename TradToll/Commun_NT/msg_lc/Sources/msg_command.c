...
#define MSG_COMMAND_NB_FIELDS MSG_HEADER_NB_FIELDS + 5
...
MSG_DECLARE_REF(MSG_COMMAND, MSG_COMMAND_NB_FIELDS)
...
PROTECTED void MSG_COMMAND_Init(void)
{
    ...
    union MSG_Field LOCAL_MSG_COMMAND[MSG_COMMAND_NB_FIELDS] =  
    {
		{ MSG_FIELD_HEADER(MSG_COMMAND, MSG_COMMAND_CD) },

        { MSG_FIELD_DWORD,  MSG_OFFSET(MSG_COMMAND, body.cmd_type),						0UL,           99UL },
        { MSG_FIELD_DWORD,  MSG_OFFSET(MSG_COMMAND, body.compl_information_lenght),     0UL,           MSG_COMMAND_COMP_INFO_LENGTH },
		{ MSG_FIELD_RAW,    MSG_OFFSET(MSG_COMMAND, body.compl_information),			MSG_OFFSET(MSG_COMMAND, body.compl_information_lenght)},
    ...
    MSG_INIT_REF(MSG_COMMAND,                    LOCAL_MSG_COMMAND);
}
...
PUBLIC struct MSG_COMMAND *MSG_COMMAND_New(void)
{
    // la variable MSG_COMMAND de type union MSG_Field[] est globale au module 
    // et a ete declare et initialise par les macros MSG_DECLARE_REF et MSG_INIT_REF
    return MSG_New(MSG_COMMAND, sizeof(struct MSG_COMMAND), "MSG_COMMAND");
}
...
PUBLIC BOOL MSG_COMMAND_Delete_All(struct MSG_COMMAND *p_command)
{
    return MSG_Delete_All(&p_command);
}

// LECTURE / ECRITURE

// passe a la moulinette une structure de donnée "struct MSG_COMMAND"
// a travers des parcours récursif des tableaux de description des champs
// de type union MSG_Field[](cf variable MSG_COMMAND de MSG_COMMAND_New
// et MSG_COMMAND_Init)
// La variable p_msg contient en retour le mesasge sous forme ASCII
PUBLIC BOOL MSG_COMMAND_Write(struct MSG_COMMAND *p_command, 
                               BYTE *p_msg, 
                               DWORD msg_size_max, 
                               DWORD *final_msg_size)
{
    *final_msg_size = MSG_Write(p_command, p_msg, msg_size_max);
...
// de type struct MSG_COMMAND
PUBLIC BOOL MSG_COMMAND_Read(struct MSG_COMMAND *p_command, 
                              BYTE *p_msg, 
                              DWORD msg_size_max, 
                              DWORD *final_msg_size)
{
    *final_msg_size = MSG_Read(p_command, p_msg, msg_size_max);
...
PUBLIC BOOL MSG_COMMAND_New_Record(HLIST *hList)
{
    return MSG_New_Record(hList, 
                         (MSG_Compare) MSG_HEADER_Compare_CD, 
                         (PVOID) MSG_COMMAND_CD, 
                          MSG_COMMAND, 
                          sizeof(struct MSG_COMMAND), "MSG_COMMAND");
}
