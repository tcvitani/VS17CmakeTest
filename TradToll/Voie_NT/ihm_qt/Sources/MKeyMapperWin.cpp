...
QKeyEvent * MKeyMapperWin::translateToKeyEvent(const MSG &msg)
{
    bool k0 = false;
    bool k1 = false;
 ...
 //   } 
    ...
    if (msgType == WM_CHAR) 
	{
        ...
        if (!ch.isNull())
		   ...
		   return createKeyEvent( QEvent::KeyPress, 0, Qt::KeyboardModifier(state),
                             s, false, 0, scancode, vk_key, nModifiers);
    ...
    else if (msgType == WM_IME_CHAR) 
	{
        ...
        if (msgType == WM_KEYDOWN || msgType == WM_IME_KEYDOWN || msgType == WM_SYSKEYDOWN) 
		{
            ...
            QChar uch;
            if (PeekMessage(&wm_char, 0, charType, charType, PM_NOREMOVE)) 
			{
                // Found a ?_CHAR
...
//                 key_recorder.storeKey(msg.wParam, a, state, text);
				   ...
				   return createKeyEvent(QEvent::KeyPress, code, Qt::KeyboardModifier(state),
                                     text, false, 0, 
									 scancode, msg.wParam, nModifiers);
...
QKeyEvent * MKeyMapperWin::createKeyEvent(QEvent::Type type, int code, Qt::KeyboardModifiers modifiers,
                             const QString &text, bool autorepeat, int count,
                             quint32 nativeScanCode, quint32 nativeVirtualKey, quint32 nativeModifiers)
{
	QKeyEvent * retEvent;

	retEvent = new QKeyEvent(type,code,modifiers, text, autorepeat, count);
