...
bool MTracer::initData(int iMaxTraceSize)
{
	...
	if(m_bErrEnabled)
	{
		...
		m_hError = CreateFile((LPCWSTR)m_sFullErrFileName.utf16(),
	...
	if(m_bTrcEnabled)
	{
		...
		m_hTrace = CreateFile((LPCWSTR)m_sFullTrcFileName.utf16(),   // open
								GENERIC_WRITE,                // open for writing 
								FILE_SHARE_READ,               
								NULL,                         // no security 
								OPEN_ALWAYS,                  // open or create 
				   ...
				   bool bErrEnabled)
{
	...
	if(sTrcFilePath.isEmpty()||sErrFilePath.isEmpty())
	{
									 ...
									 (LPWSTR)szAppDataPath)))
		{
			...
			{
				sTrcFilePath = QString::fromWCharArray((const wchar_t*)szAppDataPath);
				sTrcFilePath.append(APP_SETTINGS_SUB_FOLDER);
			}	

			if(sErrFilePath.isEmpty())
			{
				sErrFilePath = QString::fromWCharArray((const wchar_t*)szAppDataPath);
	...
	{
		qInstallMessageHandler((QtMessageHandler)MTracer::myMessageOutput);
		return TRUE;
	}
...
void MTracer::myMessageOutput( QtMsgType type, const QMessageLogContext& context, const char *msg )
{
	switch(type)
	{
	case QtWarningMsg:
