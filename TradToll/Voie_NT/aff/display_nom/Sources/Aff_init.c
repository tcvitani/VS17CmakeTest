...
EXPORT enum_instance_result WINAPI AFFLance( char * pcKey, char * pcBalNam, noyau_bal_id * piBalId )
{
	...
	// MaxLinkError
	if ( REG_Lire_Entier(CSR_REG_KEYi_ROOT, pcKey, MOD_REG_KEYv_MAX_LINK_ERROR, &AFF[ident_aff].MaxLinkError) != ERROR_SUCCESS )
	{
		AffFichierDebug(ident_aff,"AFF_INIT ***** AffLance() => Erreur registre : [%s]:%s", pcKey, MOD_REG_KEYv_MAX_LINK_ERROR);
		return INST_INIT_ERR_REGISTRE;
	}
	...
	if(REG_Lire_Entier(	CSR_REG_KEYi_ROOT,
						pcKey,
						MOD_REG_KEYv_BACKGROUND,
						&AFF[ident_aff].uiBackground) != ERROR_SUCCESS ||
						AFF[ident_aff].uiBackground != 0 && AFF[ident_aff].uiBackground != 2)
	{
		AFF[ident_aff].uiBackground = 0;

		AffFichierDebug(ident_aff,
						"AFF_INIT ***** AFFLance() => Error registry : [%s]:%s , value will be set to default value %d",
						pcKey,
						MOD_REG_KEYv_BACKGROUND,
						AFF[ident_aff].uiBackground);
	}
	...
	if(REG_Lire_Entier(	CSR_REG_KEYi_ROOT,
						pcKey,
						MOD_REG_KEYv_POLL_TIMEOUT_MS,
						&AFF[ident_aff].pollTimeoutMs) != ERROR_SUCCESS)
	{
		AFF[ident_aff].pollTimeoutMs = 10000;
